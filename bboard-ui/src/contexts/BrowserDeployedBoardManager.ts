...
export type InitialAPI = DAppConnectorAPI & {
  connect?: (networkId: string) => Promise<ConnectedAPI>;
};

export type ConnectedAPI = DAppConnectorWalletAPI & {
  serviceUriConfig?: () => Promise<{ indexerUri: string; indexerWsUri: string; proverServerUri: string; substrateNodeUri: string }>;
  getConfiguration?: () => Promise<{ indexerUri: string; indexerWsUri: string; proverServerUri?: string; substrateNodeUri?: string }>;
...
import { type BBoardPrivateState } from '../../../contract/src/index';
...
const initializeProviders = async (logger: Logger): Promise<BBoardProviders> => {
  ...
  const config = (await connectedAPI.getConfiguration?.()) ?? (await connectedAPI.serviceUriConfig?.()) ?? {
  ...
  const shieldedAddresses = (await connectedAPI.getShieldedAddresses?.()) ?? {
    shieldedCoinPublicKey: '00',
    shieldedEncryptionPublicKey: '00',
  };
  return {
    ...
    walletProvider: {
      ...
      balanceTx: async (tx: UnboundTransaction, ttl?: Date): Promise<FinalizedTransaction> => {
        try {
          ...
          const received = connectedAPI.balanceUnsealedTransaction
            ? await connectedAPI.balanceUnsealedTransaction(serializedTx)
            : await (connectedAPI as any).balanceTransaction?.(serializedTx);
          return Transaction.deserialize<SignatureEnabled, Proof, Binding>(
            'signature',
            'proof',
            'binding',
            fromHex(received.tx ?? received),
          );
        } catch (e) {
    ...
    midnightProvider: {
      submitTx: async (tx: FinalizedTransaction): Promise<TransactionId> => {
        await connectedAPI.submitTransaction(toHex(tx.serialize()) as any);
  ...
  return firstValueFrom(
    fnPipe(
      ...
      concatMap(async (initialAPI) => {
        const connectedAPI = (initialAPI.connect ? await initialAPI.connect(networkId) : await initialAPI.enable()) as ConnectedAPI;
        const connectionStatus = connectedAPI.getConnectionStatus ? await connectedAPI.getConnectionStatus() : true;
